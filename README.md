...
- **Saccade and fixation detection:** joint velocity, acceleration, amplitude, and duration thresholds; endpoint computation; merge of noise-split saccades; task epoch segmentation
...
Every analysis parameter lives in the **settings** struct, where you can customize. No thresholds are set inside the processing functions.
...
It is a viewer only: it does not run or modify the pipeline, and detected events cannot be edited from it. Selecting a stage the loaded recording has not been through says so on the axes instead of erroring — `run_demo` covers the gaze pipeline, so **Baseline corrected data** is one of these.
